...
public class MyImageView extends AppCompatImageView {
    ...
    private void resetDrawable() {
        mCnSvgBig = new Cnsvgbig(getContext(), mCnMap);
        Drawable drawable = new SVGDrawable(mCnSvgBig);
...
//                }
    ...
    private int getTouchFlag(int x, int y) {
        for (int i = 0; i < mCnSvgBig.mRegionList.size(); i++) {
            if (mCnSvgBig.mRegionList.get(i).contains(x, y)) {
                return i;
            }
        }
        return -1;
